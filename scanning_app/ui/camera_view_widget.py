...
from config import (
    ...
    ZOOM_MAX_LEVEL,
    ROI_MIN_SIZE,
    COORD_LABEL_INITIAL_TEXT,
    MOUSE_POS_LABEL_TEXT,
    CLEAR_ROI_BUTTON_TEXT,
    ZOOM_IN_BUTTON_TEXT,
    ZOOM_OUT_BUTTON_TEXT,
)
from devices.camera import ToupCamCamera as Camera
...
class CustomGraphicsView(QGraphicsView):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._panning:
            ...
            if w > ROI_MIN_SIZE and h > ROI_MIN_SIZE:
                ...
                pen = QPen(ROI_PEN_COLOR, ROI_PEN_WIDTH)
                brush = QBrush(ROI_BRUSH_COLOR)
    ...
    def wheelEvent(self, event):
        if event.angleDelta().y() > 0:
            ...
            self._zoom -= 1
        if self._zoom > ZOOM_MAX_LEVEL:
            self._zoom = ZOOM_MAX_LEVEL
        elif self._zoom < ZOOM_MIN_LEVEL:
            self._zoom = ZOOM_MIN_LEVEL
        else:
...
class CameraViewWidget(QWidget):
    def __init__(self):
        ...
        self.coord_label = QLabel(COORD_LABEL_INITIAL_TEXT)
    ...
    def init_ui(self):
        ...
        self.clear_roi_button = QPushButton(CLEAR_ROI_BUTTON_TEXT)
        self.clear_roi_button.setFixedHeight(BUTTON_HEIGHT)
        self.zoom_in_button = QPushButton(ZOOM_IN_BUTTON_TEXT)
        self.zoom_in_button.setFixedWidth(40)
        self.zoom_out_button = QPushButton(ZOOM_OUT_BUTTON_TEXT)
        ...
        coord_layout.addWidget(QLabel(MOUSE_POS_LABEL_TEXT))
